...
def update_accuracy_log(new_entry: dict) -> None:
    ...
    def rolling_hit_rate(days: int) -> float | None:
        ...
        if not window:
            return None
        return round(sum(e["hitRate"] for e in window) / len(window), 4)
...
def main():
    ...
    for ticker, pred in predictions.items():
        ...
        pred_date_close = None
        for sector_file in [
            os.path.join(HISTORICAL_DIR, f)
            for f in os.listdir(HISTORICAL_DIR)
            if f.endswith(".json") and f != "manifest.json"
        ]:
            with open(sector_file) as ff:
                sector_data = json.load(ff)
            if ticker in sector_data.get("stocks", {}):
                for candle in reversed(sector_data["stocks"][ticker].get("candles", [])):
                    if candle.get("date") == pred_date.isoformat():
                        pred_date_close = float(candle["close"])
                        break
                if pred_date_close is not None:
                    break
    ...
    report = {
        ...
        "metrics":       metrics,
        "detail":        scored[:200],  # keep first 200 for space; full metrics in summary
    }
