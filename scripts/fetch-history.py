...
Incremental mode (automatic):
    If data/historical/manifest.json exists and lastFullFetch is within the
    last 24 hours, only the last 5 days are fetched and merged.
    Otherwise a full 1-year fetch is performed.
...
INCREMENTAL_WINDOW_HOURS = 24  # re-use existing data if lastFullFetch < this
...
def is_incremental_mode(manifest: dict) -> bool:
    """Return True if a full fetch happened within the last 24 hours."""
    last_full = manifest.get("lastFullFetch")
    if not last_full:
        return False
    try:
        last_dt = datetime.fromisoformat(last_full.replace("Z", "+00:00"))
        age = datetime.now(timezone.utc) - last_dt
        return age < timedelta(hours=INCREMENTAL_WINDOW_HOURS)
...
def extract_candles(df: pd.DataFrame, symbol: str) -> list[dict] | None:
    """
...
def main() -> None:
    ...
    incremental = is_incremental_mode(manifest)
    ...
    for batch_num, batch in enumerate(batches, start=1):
        ...
        if df is None:
            ...
            for symbol in batch:
                ...
                candles = extract_candles(df, symbol)
                if candles is None:
    ...
    print("\n" + "=" * 60)
    print(f"[fetch-history] ✅  Historical data pipeline complete")
    ...
    # Exit non-zero if more than 50% of tickers failed
    if total_tickers > 0 and len(failed_tickers) / total_tickers > FAILURE_THRESHOLD:
        print(
